...
import LaunchKitModalV2 from "@/components/deep-dive/LaunchKitModalV2";
...
export default function ProjectPage() {
  ...
  const handleGenerateLaunchKit = useCallback(async () => {
    ...
    // If we already have V2 assets loaded in memory, just show them
    if (launchKitV2) {
      return;
    }
    ...
    try {
      const checkResponse = await fetch(`/api/launch-kit/v2?savedIdeaId=${project.id}`);
    ...
    try {
      const response = await fetch("/api/launch-kit/v2", {
        ...
        body: JSON.stringify({
          ...
          savedIdeaId: project.id,
          foundation: project.deepDive?.foundation,
          growth: project.deepDive?.growth,
          financial: project.deepDive?.financial,
          checklist: project.deepDive?.checklist,
        }),
      });
  ...
  }, [project, launchKitV2, transformLaunchKitResponse]);
  ...
  if (isV2Project) {
    return (
      <div className="min-h-screen bg-charcoal-dark">
        {/* DeepDiveSectionV2 handles its own header, tabs, content, and loading */}
        <DeepDiveSectionV2
          idea={project.idea}
          ideas={[project.idea]}
          profile={profile}
